...
interface MaterialRelationRow {
  ...
  quantity: number;
  kind: "recipe" | "recycles_into" | "salvages_into" | "upgrade_cost" | "repair_cost";
}
...
export async function upsertItems(baseDir: string): Promise<Map<string, UUID>> {
  ...
  console.log(`[Items] ${unchanged.length} unchanged, ${toUpsert.length} to upsert`);
  ...
  if (toUpsert.length > 0) {
    for (let i = 0; i < toUpsert.length; i += BATCH_SIZE) {
      ...
      if (error) {
        console.error(`[Items] batch upsert error (batch ${i / BATCH_SIZE + 1}):`, error);
      }
    }
  }
  ...
  for (const item of items) {
    ...
    for (const { key, type } of relationTypes) {
      const data = item[key as keyof RawItem] as Record<string, number> | undefined;
      if (!data) {
        continue;
      }
  ...
  if (missingMaterials.size > 0) {
    console.warn(`[Items] missing material items (not in items dir): ${[...missingMaterials].join(", ")}`);
  }
  ...
  if (itemIds.length > 0) {
    await supabaseService.from("item_material_relations").delete().in("item_id", itemIds);
    await supabaseService.from("item_craft_benches").delete().in("item_id", itemIds);
    await supabaseService.from("item_compatibilities").delete().in("item_id", itemIds);
  }
  ...
  if (materialRelations.length > 0) {
    for (let i = 0; i < materialRelations.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabaseService.from("item_material_relations").insert(batch);
      if (error) {
        console.error("[Items] material_relations insert error:", error);
      }
    }
    console.log(`[Items] inserted ${materialRelations.length} material relations`);
  }
  ...
  if (craftBenchRows.length > 0) {
    for (let i = 0; i < craftBenchRows.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabaseService.from("item_craft_benches").insert(batch);
      if (error) {
        console.error("[Items] craft_benches insert error:", error);
      }
    }
    console.log(`[Items] inserted ${craftBenchRows.length} craft bench relations`);
  }
  ...
  if (compatibilityRows.length > 0) {
    for (let i = 0; i < compatibilityRows.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabaseService.from("item_compatibilities").insert(batch);
      if (error) {
        console.error("[Items] compatibilities insert error:", error);
      }
    }
    console.log(`[Items] inserted ${compatibilityRows.length} compatibility relations`);
  }
