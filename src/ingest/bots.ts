...
export async function upsertBots(
	baseDir: string,
	lookups: LookupMaps,
	...
	console.log(
		`[Bots] ${unchanged.length} unchanged, ${toUpsert.length} to upsert`,
	);
	...
	if (toUpsert.length > 0) {
		for (let i = 0; i < toUpsert.length; i += BATCH_SIZE) {
			...
			if (error) {
				console.error(
					`[Bots] batch upsert error (batch ${i / BATCH_SIZE + 1}):`,
					error,
				);
			}
		}
	...
	if (missingItems.size > 0) {
		console.warn(
			`[Bots] missing items (not in items lookup): ${[...missingItems].join(", ")}`,
		);
	}
