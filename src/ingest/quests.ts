...
export async function upsertQuests(baseDir: string, lookups: LookupMaps): Promise<Map<string, UUID>> {
  ...
  console.log(`[Quests] ${unchanged.length} unchanged, ${toUpsert.length} to upsert`);
  ...
  if (toUpsert.length > 0) {
    for (let i = 0; i < toUpsert.length; i += BATCH_SIZE) {
      ...
      if (error) {
        console.error(`[Quests] batch upsert error (batch ${i / BATCH_SIZE + 1}):`, error);
      }
    }
  }
  ...
  if (changedQuestIds.length > 0) {
    await supabaseService.from("quest_maps").delete().in("quest_id", changedQuestIds);
    await supabaseService.from("quest_objectives").delete().in("quest_id", changedQuestIds);
    await supabaseService.from("quest_other_requirements").delete().in("quest_id", changedQuestIds);
    await supabaseService.from("quest_items").delete().in("quest_id", changedQuestIds);
    await supabaseService.from("quest_relations").delete().in("quest_id", changedQuestIds);
  }
  ...
  if (missingRelatedQuests.size > 0) {
    console.warn(`[Quests] missing related quests for relations: ${[...missingRelatedQuests].join(", ")}`);
  }
  ...
  if (objectiveRows.length > 0) {
    for (let i = 0; i < objectiveRows.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabaseService.from("quest_objectives").insert(batch);
      if (error) console.error("[Quests] quest_objectives insert error:", error);
    }
  ...
  if (otherRequirementRows.length > 0) {
    for (let i = 0; i < otherRequirementRows.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabaseService.from("quest_other_requirements").insert(batch);
      if (error) console.error("[Quests] quest_other_requirements insert error:", error);
    }
    console.log(`[Quests] inserted ${otherRequirementRows.length} quest_other_requirements`);
  }
  ...
  if (relationRows.length > 0) {
    for (let i = 0; i < relationRows.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabaseService.from("quest_relations").insert(batch);
