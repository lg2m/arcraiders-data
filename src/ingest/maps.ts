...
export const mapIdEnumSchema = z.enum(["dam_battlegrounds", "buried_city", "the_spaceport", "the_blue_gate", "stella_montis"]);
...
export async function upsertMaps(baseDir: string): Promise<Map<string, UUID>> {
  ...
  console.log(`[Maps] ${unchanged.length} unchanged, ${toUpsert.length} to upsert`);
